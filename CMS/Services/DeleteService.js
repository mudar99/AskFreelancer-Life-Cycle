...
import { CmsDeleteService } from '../../API';
import axios from "axios";

class DeleteService extends Component {
    state = {
        visible: false,
        url: CmsDeleteService,
    }
    setVisible = () => {
    ...
    render() {
        return (
            <>
                <a id={`TrashIcon_S${this.props.id}`}>
                ...
                <ConfirmPopup target={document.getElementById(`TrashIcon_S${this.props.id}`)} visible={this.state.visible} onHide={() => this.setState({ visible: false })} message="هل تريد حذف الخدمة؟"
