...
class DeleteRole extends Component {
    ...
    accept = (e) => {
        axios.delete(CmsDeleteRole + this.props.id).then(
            res => {
    ...
    render() {
        return (
            <>
                ...
                <ConfirmPopup target={document.getElementById(`TrashIcon${this.props.id}`)} visible={this.state.visible} onHide={() => this.setState({ visible: false })} message="هل تريد حذف الدور"
